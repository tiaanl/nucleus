...
struct CollectorInvocation<Collector, R(Args...)> {
  bool invoke(Collector& collector,
              const std::function<R(Args...)>& callbackFunction, Args... args) {
...
struct CollectorInvocation<Collector, void(Args...)> {
  bool invoke(Collector& collector,
              const std::function<void(Args...)>& callbackFunction,
              Args... args) {
...
template <typename Signature,
          typename Collector = detail::CollectorDefault<
...
struct Signal final : detail::ProtoSignal<Signature, Collector> {
  ...
  Signal(const CallbackFunction& function = CallbackFunction())
    : ProtoSignal(function) {}
};
