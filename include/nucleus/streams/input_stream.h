...
public:
  ...
  // be able to do this, in which case they should do nothing and return false.
  // Others might be able to manage it by resetting themselves and skipping to
  // the correct position, although this is obviously a bit slow.
  ...
  // "\r\n" or end-of-stream.  After the call, the stream's position will be
  // left pointing to the next character following the line-feed, but the
  ...
  // characters from the stream until it hits a null character or end-of-stream.
  virtual std::string ReadString();

  // Read and discard a number of bytes from the stream.  Some input streams
  // might implement this efficiently, but the base class will just keep reading
  // data until the specified number of bytes have been done.
