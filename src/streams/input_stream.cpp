...
int16_t InputStream::readInt16() {
  ...
  if (read(temp, 2) == 2)
    return static_cast<int16_t>(ByteOrder::littleEndianInt16(temp));
...
int32_t InputStream::readInt32() {
  ...
  if (read(temp, 4) == 4)
    return static_cast<int32_t>(ByteOrder::littleEndianInt32(temp));
...
int64_t InputStream::readInt64() {
  ...
  if (read(n.asBytes, 8) == 8)
    return static_cast<int64_t>(ByteOrder::swapIfBigEndian(n.asInt64));
